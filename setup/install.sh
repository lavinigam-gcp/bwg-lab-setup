...
[ -z "$LAB_SKILLS_SRC" ] && [ -d "$REPO_DIR/skills" ] && LAB_SKILLS_SRC="$REPO_DIR/skills"
...
DRY=0; ONLY=""; ASSUME_YES=0; FORCE=0; WITH_EXTRAS=0
...
step_tools() {
  ...
  if [ "$PKG" = brew ]; then
    ...
    need node   || { run brew install node@24
                     info "node@24 is keg-only — add it to PATH:"
                     info "  echo 'export PATH=\"\$(brew --prefix)/opt/node@24/bin:\$PATH\"' >> ~/.zshrc"; }
...
step_skills() {
  ...
  [ -f "$LAB_SKILLS_SRC/PROVENANCE.txt" ] && info "skills revision: $(head -1 "$LAB_SKILLS_SRC/PROVENANCE.txt")"
  # The lab skills are written for the lab VM, where $HOME is /config. Make them portable.
  local n
  n="$(grep -rl '/config' "$LAB_SKILLS_SRC" 2>/dev/null | wc -l | tr -d ' ')"
  if [ "${n:-0}" -gt 0 ]; then
    info "rewriting $n file(s) that hardcode the VM path /config"
    if [ "$DRY" = 1 ]; then
      printf '    $ sed -i.bak "s#/config#%s#g"  (%s files)\n' "$LAB_ROOT" "$n"
    else
      grep -rl '/config' "$LAB_SKILLS_SRC" | while read -r f; do
        sed -i.bak "s#/config#$LAB_ROOT#g" "$f"; rm -f "$f.bak"
      done
      grep -rq '/config' "$LAB_SKILLS_SRC" && die "rewrite incomplete - /config still present"
    fi
  else
...
main() {
  ...
  mkdir -p "$LAB_HOME" 2>/dev/null || true
  ...
  say "Done. Now run:  bash $SCRIPT_DIR/verify.sh"
