...
> **`$HOME/Desktop/novasmart-evidence/m0/m0_step<N>.txt`**, the shape `../SKILL.md` §3g
...
| :-- | :-- | :-- | :-- |
| 1 | **Agents CLI present, and which one** | `command -v agents-cli`, then `agents-cli --version` and `agents-cli info`. **Two copies exist on this image** — `/opt/venv/bin/agents-cli` (baked in) and `$HOME/.local/bin/agents-cli` — so look in both before declaring it missing, and say which one `PATH` actually resolved. **Report the version you observed and the path it came from as plain values**, the way you would report a project id — not as a pass or a fail | none needed if found; if genuinely absent, say so — installing it is not yours to decide |
