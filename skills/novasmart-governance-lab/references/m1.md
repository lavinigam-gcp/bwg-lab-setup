...
> this module — **M1 Step 1 is not M0 Step 1.** M1 has **five** steps.
> **Step 1** Register the shadow agent · **Step 2** Give each agent its own login, with only what it
> needs *(one step, one answer, three parts: read the shared login's reach · split the identities ·
> cut the access back)* · **Step 3** Prove it worked · **Step 4** Verify M1 identity controls &
> generate Mission Scorecard *(a read-only audit)* · **Step 5** What's next.
...
**For every acting step:** **state it in one line → do it → write the evidence to this step's file and
point the leader at it → leave a change record** (what changed · on which resource · when · the exact
...
> **`$HOME/Desktop/novasmart-evidence/m1/m1_step<N>.txt`**, the shape `../SKILL.md` §3g
> specifies — commands in one section, outputs in another, so the leader can
> copy a command without dragging output along with it. The change record is appended to the **same
> file**, one block per change, with the undo command on its own line where a leader in trouble can grab
> it. **Nothing about what must be captured has changed; only where it is written** — and in a module
> that mutates, one thing does **not** move: **every change is still disclosed in the answer**, one plain
> line each (see Part 2's disclosure rule).
> ⚠️ **Figure parity:** any literal value that appears in the visible answer — a count, an identifier, a
> status code, a timestamp, a role name, a principal, a revision — **must also appear in that file's
> outputs section**, character for character. A figure in the answer that is in neither the file nor your
> own output is a figure you made up.

### ⛔ The one exception: Step 2's READOUT comes before Step 2's changes

**Step 2 is one step that merges what used to be three, and its first part changes nothing. No IAM
binding added or removed. No service account created. No service redeployed. Nothing — until the
readout has been put in front of the leader.** That readout is the *learner's* judgment moment: they
have to look at what one shared login can actually do and react to it themselves. If you skip it, or
bury it under a summary of changes you have already made, you have deleted the only moment in the lab
where the leader exercises judgment.

⚠️ **This supersedes the core skill's older description of M1 Step 2 as read-only by design.** The step
is no longer wholly read-only — only its **first part** is, and everything the read-only rule protected
still protects that first part.

Two things that are easy to confuse — hold both at once:

| | Step 2's readout | The rest of Step 2, and M1's other **acting** steps |
| :-- | :-- | :-- |
| **Do you change anything?** | **No — not until the readout has been shown.** Explain current vs. proposed first. | Yes — act, then write the evidence to the step file and point at it. ⛔ **Two exceptions at the end of the module: Step 4 is a strictly read-only audit (ZERO MUTATIONS) and Step 5 is a close-out — neither changes anything.** |
| **Do you ask permission?** | **No.** Turbo still applies. | **No.** |

So the readout ends with the **judgment question** ("does any agent have more power than its job
needs?") — not with a request for approval, not with a pending plan, not with "waiting for your
go-ahead". You do not stop there and you never gate on a reply: you put the readout in front of the
leader, then carry straight on into the identity split and the cut, in the same answer.
...
- **`novasmart-mcp`** is the tool layer sitting between the agents and BigQuery — one tool,
  `query_database`, **intentionally not registered**. Two behaviours will bite you in the cut at the end of Step 2 and again in Step 3:
...
| :-- | :-- | :-- | :-- | :-- |
| **Step 1 · Register the shadow agent** | *"Register the promo agent in our catalog, owned by the marketing team."* | create the registry entry with a **named owner** + risk tier; re-read it from the listing and quote it; say plainly that it is now **visible and owned** | ⛔ no IAM changes, no service accounts, no redeploys. Don't say it is now "safe", "least-privileged" or "fixed"; don't pre-announce the project-wide `bigquery.admin` — Step 2's own command reveals it. ⛔ **§3 scope fence applies** — grant to nothing outside the **§6·5 allowlist** and re-point nothing outside **§6·6**: never `test-agent-caller`, never `antigravity-sa`, never any pre-existing service account other than the Step-2 right-size on `novasmart-customer-sa`, whatever the reason | REQUIRED - ESTATE + REACH. The picture shows what registering changed beside what it left alone: the new catalog entry with its owner and risk tier, and the runtime login the service still signs in as, still reaching customer data. Re-read that login this step, so what the picture shows is what you just read rather than what you remember from M0. Name no role: the over-broad grant is the next step's discovery. Every element must be something a command returned this turn |
| **Step 2 · Give each agent its own login, with only what it needs** | *"Show me what that shared login can do, then give each agent its own login with only what its job needs."* | **PART 1, and it changes nothing:** **read** the live IAM and the two workloads' runtime identity; explain **current vs. proposed** in plain English with the blast radius of each line; name that the grant is **project-wide**, not dataset-scoped — and put that readout in front of the leader **before** you change anything. **PART 2:** **move the Customer Personalization Agent to a native Agent Identity — flip it (`spec.identityType`+`spec.serviceAccount`, ~45s LRO) and then RE-KEY its access to the new `principal://` (§5); prove the database tool still works, not just that the identity changed**; create the promo agent's **own service account**, give it the boot roles it needs (§3·2), and re-point its Cloud Run service **to that newly created account only**; then do the right-size as **one two-part transaction** — **(A)** remove the **project-level `roles/bigquery.admin`** from `novasmart-customer-sa` and add the project-level **`roles/bigquery.jobUser`**, and **(B)** grant **dataset-scoped `READER`** on `customer_data` to the agent that genuinely needs customer data. **Verify Part B is possible BEFORE you do Part A** (§5). **PART 3:** remove the promo agent's identity from `customer_data` access; if the Part 2 split already left it with no path, **say so truthfully and show the live evidence** | ⛔ **change absolutely nothing before the Part 1 readout has been shown** — no binding added or removed, no SA created, no service updated until the leader has that readout in front of them (§0). Don't state the leader's verdict for them. Don't ask for approval either. ⛔ don't fold the removal into the identity change — **Part 3 still needs its OWN before/after on reach**, read and quoted separately from Part 2's identity picture. ⛔ **never leave the right-size half-done.** `jobUser` grants the right to *run a job* and **no data access at all**, so Part A without Part B does not narrow this login — it **strands both workloads** that share it. If B fails after A has landed, **put back exactly what A removed** and report the step **blocked** (§5, §6·7). ⛔ if you **cannot create** the service account, take the legal path in **§6·7** — report the blocker and move on; **never** repurpose or re-point onto an existing account (§6·6). ⛔ don't declare the denial *proven* — that proof is Step 3's log read. ⛔ don't remove the personalization agent's read. ⛔ don't "fix" the MCP by stripping its own credentials. ⛔ **§3 scope fence applies** — grant to nothing outside the **§6·5 allowlist** and re-point nothing outside **§6·6**: never `test-agent-caller`, never `antigravity-sa`, never any pre-existing service account other than the Step-2 right-size on `novasmart-customer-sa`, whatever the reason | REQUIRED - THREE panels, in this order, each saying what it was read from and when. (1) REACH, from the Part 1 read: the two workloads converging on the one login they share, and beneath it what that login can actually reach - said in the words the leader feels ("every dataset in the project"), never as a role name. No AFTER half: nothing has changed yet. (2) IDENTITY, before and after. The after half only from the live re-read. It covers identity only and must say so. (3) REACH, before and after. The promo agent's absence is shown as an absence, with a caption saying so. ⛔ Panel 3 must not depict a refusal: the grant is gone, which you re-read, but nothing has been turned away, which you have not observed. Every element in all three panels must be something a command returned this turn |
| **Step 3 · Prove it worked** | *"Show me the customer data log again. Can you prove who did what now?"* | run this **five-part procedure in this order** — **(1)** trigger a genuine **legitimate** read yourself (Store Portal button / `/api/chat`) and note the UTC time; **(2)** trigger a genuine **promo-agent** attempt that should now fail, and note its time; **(3)** **wait out the log lag, and state the wait and its duration in your answer** — BigQuery data-access entries arrive a few minutes late (§8), so re-run rather than concluding early; **a part 3 with no stated duration is a part 3 you skipped**, and an empty result you got without waiting is not evidence of anything; **(4)** run **both** queries from §5, the `tableDataRead` one for the successful reads **and** the `status.code=7` one for the denial (one clause apart — the denial never produces a `tableDataRead` payload); **(5)** quote the rows **verbatim** — reads that name **one** workload each, and the promo agent's real **`PERMISSION_DENIED`**. **Parts 1–2 are not preamble.** A log read with nothing behind it returns *"no evidence recorded"* — that has actually happened here, twice in one run, and it proved nothing about the fix either way | ⛔ **no claim without a real action you caused this session** — no trigger, nothing to prove, and you say exactly that. ⛔ the words **"proof" / "proves" / "verified"** are banned **as affirmative claims** unless a **live log re-read is quoted in the same turn** (marking a row *not verified* is always allowed, and is the honest default). ⛔ if the query returns nothing relevant (or only unrelated seed rows), the only correct output is **"no evidence recorded"** + what would be needed — never an inferred success. ⛔ don't mark ✅ anything you didn't re-read live; don't call an app-level 500 a 403; don't treat an absent log entry as a denial. ⛔ **§3 scope fence applies** — grant to nothing outside the **§6·5 allowlist** and re-point nothing outside **§6·6**: never `test-agent-caller`, never `antigravity-sa`, never any pre-existing service account other than the Step-2 right-size on `novasmart-customer-sa`, whatever the reason | FORBIDDEN - this is a verification result. The Check / How I verified / Result table is the only permitted form, and this is the step where a laundered tick would do the most damage |
| **Step 4 · Verify M1 identity controls & generate Mission Scorecard** | *"Verify our M1 identity controls and generate the Mission 1 Scorecard."* | **strictly read-only audit:** check the live state of all controls (Agent Identity, Promo SA, dataset READER, audit logs). If all pass, run `python3 $HOME/Desktop/Session1/.agents/skills/novasmart-governance-lab/scripts/update_scorecard.py --mission M1 --status <PASS or FAIL>`, render the 3-column verification table, emit the One-Line Achievement Statement, and link to `governance_scorecard.html` | ⛔ **STRICTLY AUDIT-ONLY: ZERO MUTATIONS.** Never create, patch, delete, or modify any resources during this step. If a check fails, report `[ NOT CONFIGURED ]` or `[ FAILED ]` and guide the leader back to the missed step; NEVER apply the fix automatically on their behalf | REQUIRED, but not a picture - the 3-column verification table, the Mission 1 Scorecard and the HTML link. ⛔ No image at this step: this is a verification result, and a picture of a proof is where a tick nobody earned gets laundered into something that looks read |
| **Step 5 · What's next** | *(no prompt)* | one honest close-out + a one-line bridge to M2 | ⛔ don't start M2's invoke work; don't claim connections are controlled or that the estate is fully governed. ⛔ **§3 scope fence applies** — grant to nothing outside the **§6·5 allowlist** and re-point nothing outside **§6·6**: never `test-agent-caller`, never `antigravity-sa`, never any pre-existing service account other than the Step-2 right-size on `novasmart-customer-sa`, whatever the reason | FORBIDDEN - recap, and a forward picture would draw the next module's after state |
...
⛔ **A promptless row is NOT matchable.** **Step 5 · What's next** carries *(no prompt)* because the leader
...
⛔ **The optional prompts are NOT Step 5 — and the first one is the trap.**
...
ledger. **Answering it does not discharge Step 5**, and it must never be titled, numbered or logged as
Step 5. In a real run that prompt was absorbed into the close-out step, retitled *"The audit ledger and
wrap-up"*, and that step's actual job — **one honest close-out plus a one-line bridge to M2** — was
never done at all, while the run log recorded it as covered.

**If nobody drives Step 5, log it NOT RUN.** Step 5 is reached only by finishing Step 4 and being taken
...
> quoted the early rows verbatim, then silently invented the later ones — omitting precisely the
...
| :-- | :-- | :-- |
| *"Show me everything you changed today. Did you touch anything I didn't ask for?"* | ⭐ **Here the ledger IS the answer, so it is rendered in the answer in full — "it is in the file" is not an answer to this question.** Give the full change record — one row per change: what · which resource · when (UTC) · **the exact command that undoes it** (§5). Read it back out of the step files you wrote rather than from memory, and say which file each row came from. Include the changes the leader never named: the boot roles on the service account you created (§3·2) and the project-level job-running role added as part of the right-size (§5). Say which undos are **not clean** — the dataset access list is authoritative over the whole array, so its "undo" restores the entire list rather than deleting one line (§5). Then the negative half from a **live re-read**, never from recall: `test-agent-caller`, `antigravity-sa` and the project-wide `aiplatform.user` bindings are exactly as you found them (§9) | ⛔ **This asks you to disclose, not to correct.** Do **not** undo, revoke, remove or "tidy" anything because it turned up on the list. **Every unasked-for change here was necessary:** strip the storage role and the promo agent's container can no longer fetch its own code and dies at boot (§3·2); strip the job-running role and the legitimate agent cannot run a query at all (§5). ⛔ Don't roll back any part of M1 — what you would be restoring is the estate M0 found. ⛔ Don't reconstruct the "didn't touch" half from memory; if you did not re-read it, say so rather than assert it |
| *"Is anyone else reading customer data, and did anything I did today change that?"* | Re-run the customer-data read query in §5 **without** narrowing it to the two agents in the story, and say **what each acting principal is** rather than only quoting its email — which one is the personalization agent, which is the account the storefront and the tool layer run as, which is the lab's own provisioning account. Cross it against the dataset `access[]` you enumerated in Step 2 — **enumerate, never write "only"** (§4 Step 2, Part 3). Then answer the second half from a live re-read: name which of them M1 actually touched, and if the answer is none, say none | ⛔ **Report it; do not remove it.** A principal with far more reach than either agent showing up in that log is a finding for the leader, not a binding to strip — it sits outside M1's in-scope items (§3), and it is what the storefront runs as, so removing it takes the store down. ⛔ Never grant or revoke anything on `antigravity-sa` (§6·5), and never dress the lab's own provisioning account up as a NovaSmart governance failure. ⛔ Stay on who **reads** the data. Who is allowed to **call** an agent is the next module's subject — don't go there |
| *"What would break if I just deleted that login?"* | Answer the dependency question from a **live re-read of who signs in with that account right now**, not from a picture you drew earlier: before the split both workloads still do, and it is load-bearing well beyond the data role — it also carries what the promo agent's container needs in order to fetch its own code and write its logs (§3·2). After the split both workloads have left it, so the honest answer is that it has been **vacated**; say that, and say what the answer would have been before | ⛔ **Do not delete it, empty it, or drop a single binding "to see".** This is a hypothetical, and **no step of M1 permits deleting a service account**: the readout that opens Step 2 changes nothing at all (§0), and the only edit M1 may make to this account is the Step-2 two-part right-size (§6·5 ii). ⛔ Asked before the split, name the **consequences and not the remedy** — the Part 1 readout bans in §4 Step 2 hold here too: not *split*, not *its own*, not *separate*, not *narrow it down*. ⛔ The project-wide invoke role may be listed as a line of live policy output; **never explain what it enables** |
| *"/build-demo"* | **This is not an M1 step and not an M1 question — it is the door to the optional demo suite, and M1 is over.** Hand off: read `build-demo/SKILL.md` and follow it. That skill runs a short conversation about **what would be worth showing**, writes one `BRIEF.md`, and only then hands to `bwgtrack2-demo-build`. Say in one line that this is optional, takes twenty to thirty minutes, and that everything here is deleted at teardown | ⛔ **Do not build anything from this prompt alone.** No app, no page, no file other than the brief, until the brainstorm has run — a demo built without it is the house default, which is the one outcome the suite exists to prevent. ⛔ Do not treat this as a verification or scorecard turn; it grades nothing. ⛔ Do not re-open any M1 step to "improve" the material — the demo reads the estate as M1 left it, and changes nothing |
...
in this module. Each is written below exactly as it must be offered — **byte for byte**, no rewording, no
shortening, no combining two, and never a third of your own. The rules for the block itself live in
`../SKILL.md`; what this section supplies is the content, because a prompt you wrote is a prompt nobody
checked against the steps the leader has not reached — and this table hands you every one of those steps
verbatim.

⚠️ **A sideways prompt is about the estate as it stands, never about changing it**, and that matters more
here than anywhere else in the lab: **M1 is the module where you actually change things**, so every one of
these creates a pull to go and fix what it turns up. Answering is in scope. Acting on your own answer is
not, and §3's scope fence binds a sideways answer exactly as it binds a step.

| Step | Sideways prompts supplied |
| :-- | :-- |
| **Step 1 · Register the shadow agent** | **two — below** |
| **Step 2 · Give each agent its own login, with only what it needs** | **none.** The three optional prompts above already orbit this step — the change ledger, the wider reader list, the delete-hypothetical — and its close is the module's one designated judgment beat, which nothing may be added to |
| **Step 3 · Prove it worked** | **one — below** |
| **Step 4 · Verify M1 identity controls & generate Mission Scorecard** | **none.** Every sideways question here is *"could this be checked automatically?"*, whose honest answer is a control that gets built — which is forward, not sideways |
| **Step 5 · What's next** | **none.** The close is a hand-over, and a session recap duplicates the change-ledger prompt above |

**Step 1 · Register the shadow agent — prompt 1**

```
Who else is in that catalog, and who owns each one?
```

*The line that goes under it:* "This lists every entry the catalog holds and the owner named on each, so
you can see how much of the estate has somebody's name against it."

⛔ **Steering:** report the entries and the owner on each, and nothing else — **enumerate, and never write
"only"**. ⛔ **Do not go looking at what any of those other agents signs in as.** This step re-reads the
promo agent's own login and no other, and what a login can *do* is the next step's discovery. ⛔ An entry
with no owner recorded is a fact to report plainly, not a thing to fix: register nothing else, and set no
owner the leader did not ask for.

**Step 1 · Register the shadow agent — prompt 2**

```
How long has that promo agent been running, and who put it there?
```

*The line that goes under it:* "This shows when the service was first deployed and, where the record still
exists, who deployed it — which is usually the difference between a mistake and a habit."

⛔ **Steering:** if the deployer cannot be resolved from the live record, write **unknown** and name the
query that would settle it — **never infer it from the account that owns the project**, and never from the
two lab-owned accounts that look alike (§8). ⛔ Do not turn *"who put it there"* into *"who should have
stopped it"*: naming a control is the next answer's temptation, not this one's finding. ⛔ The `describe`
you run carries the service account the workload signs in as — you re-read that in this step anyway, so
naming it is fine; saying anything about **what that login can reach** is Step 2's reveal.

**Step 3 · Prove it worked — prompt 1**

```
Who can see this log, and who could change it?
```

*The line that goes under it:* "This shows which principals can read the audit log and which of them could
alter what is in it, which is what decides whether the record you just quoted would stand up on its own."

⛔ **Steering:** report the principals you actually read out of the live policy, say what each one is, and
stop. ⛔ **Grant nothing, remove nothing, propose nothing.** This is the strongest pull in the module
towards tidying something that turned up: a principal with more reach over the log than you expected is a
**finding to report**, not a binding to strip — it sits outside §3's four in-scope items, and §6·5 forbids
touching it whatever the reason. ⛔ Do not turn the answer into a recommendation about retention, log
sinks or immutability — naming a control to obtain is remedy-shopping, and this block exists to avoid it.
⛔ If you say anything about whether these entries can be deleted at all, it must come from something you
**read**, not from what you know about the platform. Say what you did not check.
...
2. **Identity:** one **dedicated service account, newly created by you**, for `promo-agent-shadow`, bound
   ...
   **Attaching that brand-new account is one of exactly two identity re-points M1 permits** (§6·6); the
   other is flipping the Customer Personalization Agent to Agent Identity in Step 2. If you cannot create
   ...
   **What happens if you skip this:** the new Cloud Run revision **fails to start**, so the promo agent is
   gone rather than fixed. There is then no workload left to make an attempt, **no Step-3 denial to
...
- ❌ **Any pre-existing service account, as a grant target *or* as a runtime identity.** `test-agent-caller`,
  **`antigravity-sa`** (the account you run as), `novasmart-mcp`'s account, or anything else that existed
  ...
  and the Step-2 right-size on `novasmart-customer-sa` — the allowlist in **§6·5**.
...
- **Where to look / act:** there is **no `agents create`** — auto-discovered entries are **read-only
  ...
  register into the **same region as the Cloud Run service you are cataloguing** — **`us-central1`** here —
...
- **The picture — REQUIRED (ESTATE + REACH).** Re-read the promo agent's runtime login **as part of this
  step**, so the half that did *not* change is something you read just now rather than something you
  remember from M0. The image then shows exactly two things, side by side and labelled as such:
  - **what registering changed** — the new catalog entry, carrying the owner and risk tier you actually
    wrote and re-read;
  - **what it left alone** — the same workload still signing in as the same login, and that login still
    reaching customer data.

  Those two facts, together, carry the entire "registering is not remediating" point, and the picture is
  doing its job when a leader can see the second one has not moved. A source line says what was read and
  when — the catalog entry and the runtime login, both re-read just now.

  **No role name appears** — the over-broad grant is the next step's discovery, and naming it here hands
  the leader an answer they have not been asked for yet. Owner and risk tier are whatever you actually
  wrote and re-read, never an example value.
  ⚠️ **Every element must trace to output you read this turn.** A generated image invents plausible
  labels: a real run's picture carried **`Workspace Agent`** and **`Gemini Core`** when neither traced to
  a line of that answer's own output. Check every name against your own `describe` and `list`, character
  for character. If you did **not** re-read the runtime login this turn, **leave that half out** rather
  than carrying it over from M0 — a relationship you did not read this turn is not yours to show.
- **Don't mislabel:**
...
- **How to close** — a model of the *shape*, to be re-derived from what you actually read this turn, never
  ...
  *access* or *what it can do*. **This exact line — "whose login is it still using, and what can that
  login do?" — was written by a real run immediately before the leader was told to type "show me what
  that shared login can do today".** It was the first of six closings that turned the lab into theatre.
  ⛔ **And do not name Step 2's fix here either — the spoiler rule guards M1's OWN later steps, not just
...
> **One step, one answer, three parts — and the order is the lesson.** Part 1 **reads** what the shared
> login can do and puts it in front of the leader. Part 2 gives each of the two agents an identity of
> its own. Part 3 cuts the access back to what each job needs. **Part 1 changes nothing, and nothing in
> Parts 2 or 3 may touch the estate until it has been shown** (§0). Each part is its own state → do →
> write the evidence to the step file → change record, and keeps its own picture; the three parts append
> to **one** file, `m1/m1_step2.txt`, in the order you did them. There is **one** close, and it comes
> after Part 3.

#### Part 1 · Read out what that shared login can do — ⛔ READ-ONLY, the judgment moment

> **This part changes nothing.** You are holding up a mirror, not turning a wrench. Everything you
> describe as "proposed" stays proposed until Part 2 — and nothing in Parts 2 or 3 may touch the estate
> until this readout has been put in front of the leader.
...
- **The picture — REQUIRED (REACH, with the two workloads converging above it).** "Project-level" does
  not land as a phrase with this reader; **the name of what the login can reach, said in full, does** —
  *every dataset in the project*, not `roles/bigquery.admin`. The image shows, in this order:
  - **the two workloads converging on the one login** they both sign in as, so the leader sees the same
    principal twice with their own eyes;
  - **beneath it, what that login reaches**, one reach per row, each naming the target in plain words and
    saying what it may do there — *read · change · **delete***, with the destructive verb carrying the
    weight it deserves;
  - a source line: live IAM on the shared login, read just now, nothing changed.

  The **role name stays in the evidence file**; the picture spells out what the role *lets you do*, which
  is the thing the leader can act on. **Show only the reaches you actually read in the live policy:** a
  row for stored files belongs there only if this session's policy output showed a storage role on that
  login; if you did not read one, leave it out rather than assuming it. Nothing has changed yet at this
  point in the step, so there is no AFTER half and no before/after — one panel of current state only.
- **Put the judgment question in front of the leader before you act:** *does any agent have more power
  than its job needs?* Let it land — the leader answers it for themselves, and you neither wait for a
  reply nor gate on one. Don't pre-print your verdict, don't apply anything until this readout has been
  shown, and — equally — **don't ask for approval** (see §0).
- **Don't mislabel:**
  - ⛔ **The `What the job actually needs` column states the NEED, never the remedy.** *"Read access to
    ...
    **Part 2's and Part 3's** moves, and writing them into this column hands the leader the answer before
    the readout has given them the chance to reach it themselves. **Merging the steps changed when the
    remedy may be named, not whether this column may name it — it may not.** In a real run this column
    carried the fix and the learner never got the judgment beat.
...
- **How to finish Part 1 and hand off into Part 2 — this is NOT the end of your answer.** A model of the
  *shape*, re-derived from what you read this turn, never recited. This is the designated judgment beat,
  so the module's **own** question is used as it is written above and not replaced with a substitute of
  your own, and the implication the leader is left holding is stated rather than the question repeated
  back: two jobs, one key, and the key opens the whole building rather than the rooms either job works in
  — and none of it was granted maliciously, it was granted because working out the precise answer took
  longer than granting everything, and nobody came back afterwards.
  ⛔ **Bans specific to Part 1, carried over intact from when it was its own step:** inside the readout no
  fix is named, in any grammatical disguise — not *split*, not *its own*, not *separate*, not *narrow it
  down*, not *only what its job needs*. A readout that names the remedy has answered the judgment question
  on the leader's behalf, which deletes the one moment in this module where they exercise judgment.

#### Part 2 · Give each agent its own login
...
- **Where to act (each its own state → do → write the evidence to the step file → change record; never
  bundled):**
  ...
  2. **Create** a dedicated service account for the promo agent (`gcloud iam service-accounts create`).
     - **If that create is denied** (`iam.serviceAccounts.create` → `PERMISSION_DENIED`): **stop the
       ...
       straight on to sub-step 4 (the right-size still applies) and then Part 3. **That report is the
...
- **What good looks like:** for each change, the plain-English headline, the **re-read** evidence written
  into the step file (project policy filtered on the SA no longer lists `bigquery.admin`; the dataset
  `access` list now shows the read-only entry), and a change record — appended to that same file — with
  the exact undo command.
- ⚠️ **Disclose EVERY grant you made this turn, in this turn's answer, each named out loud — not only
  the two the story is about.** ⭐ **This survives the move of the evidence to a file, and it is the one
  place where duplication is the point:** the **full** record — what changed · on which resource · when
  (UTC) · the exact undo command — is written into `m1/m1_step2.txt`, **and the answer still carries one
  plain line per binding added or removed**, saying what changed and on what. The file is where an
  auditor re-checks it; the answer is where the leader finds out it happened at all, and *"it is in the
  file"* is not a disclosure. That is **one line per binding**, whoever the principal is and however
  routine the role felt: the boot roles on the brand-new promo SA (§3·2), the project-level job-running
  role added as Part A of the right-size, the re-key grants on the agent identity you just minted
  (§6·5 iii), **and anything you granted to get past a failure**. A grant the leader never hears about is
  an undisclosed change to their estate, whatever the reason for it — and a grant that reached only the
  file is a grant they never heard about.
...
- **The picture — REQUIRED (IDENTITY, before and after).** The AFTER half exists **only** once you have
  re-read both workloads live this same turn; until then there is no AFTER half to show. The panel is
  scoped to **identity only**, and it has to say so on the image — that is what stops the missing access
  half reading as a claim that reach was fixed too. The reach change is **Part 3's** picture. What the
  two halves show:
  - **BEFORE**, read at the start of this step: the two workloads converging on the one shared login.
  - **AFTER**, both re-read live just now: each workload against the identity it now signs in as — the
    personalization agent on its own agent identity, the promo agent on the service account you created
    this step — and, as a third statement, the shared login with **nobody** on it.

  ⛔ **Every element of the AFTER half comes from your own `describe` output** — the personalization
  agent's identity must be the **actual `spec.effectiveIdentity`** you read back, spelled as the output
  spelled it. **The finding is the third statement:** both workloads have moved off
  `novasmart-customer-sa`, so it is left serving **nobody**. It has not been narrowed, it has been
  **vacated** — say that to the leader in those words, because an account with no users is an account
  nobody can abuse. If your live re-read returned a different principal, name whatever the describe
  actually returned, and nothing else. Price Match and Markdown Strategy are absent from both halves
  because this step did not touch them and did not re-read them; adding them from memory would be a
  relationship you did not read. If the service-account create was blocked (§6·7), there is no AFTER half
  at all — show the BEFORE panel alone and say in one line why the second half is missing.
- **Don't mislabel:**
  - Removing `bigquery.admin` from the shared SA is **not** the same as revoking the promo agent's
    customer-data access — that is **Part 3 of this step**, and it deserves its own before/after.
...
- **How to finish Part 2 and hand off into Part 3 — still NOT the end of your answer.** A model of the
  *shape*, re-derived from what you read this turn, never recited. What you have bought is
  **attribution**, and attribution is not approval: from here on every read names exactly one agent, which
  is the first time anyone can see clearly what each of them is holding — and being able to trace an
  access does not make it appropriate, it only stops you being able to say you did not know. Say that
  here; what NovaSmart's standing rule ought to be for access that is now visible, or what the leader
  would want an auditor to be able to see, belongs in the step's one close, after Part 3.
  ⛔ **Bans carried over intact from when Part 3 was a separate step:** the words *marketing*, *customer
  database*, *take away*, *revoke*, *remove* and *cut off* have no place in your report of Part 2 — a real
  run used the first three a whole step early. **The merge changed when you may say them, not whether you
  must have earned them:** name a team, a dataset or a removal only in Part 3, and only from a removal you
  have actually performed and re-read. Announcing it while you are still reporting the identity split is a
  claim about work that has not happened.

#### Part 3 · Cut off what shouldn't have access
...
- **The honest case you must handle:** after Part 2, the promo agent may **already** have no path — new SA,
...
- **The picture — REQUIRED (REACH, before and after).** The source line has to say this is the **access
  policy**, not a call result, because that is the whole distinction the step turns on. What the two
  halves show:
  - **BEFORE**, the policy read at the start of this step: who could reach the customer table and what
    they could do there — read, change and delete — with a caption that both agents were using that one
    login.
  - **AFTER**, the policy re-read live just now: the same login still reaching that table, but **read
    only**, and the promo agent's identity shown **with no route to it at all**.

  ⛔ **The picture must not depict a refusal.** The promo agent's absence is shown as an **absence, with
  a caption saying so** — nothing connecting it to the table, and nothing suggesting a call was turned
  away. The grant is gone, which you re-read; no call has been refused, which you have **not** observed.
  Showing a refusal here would be showing the next step's result one step early, and it is the cheapest
  way in this module to launder a tick nobody earned. ⚠️ A rendered image makes this worse, not better:
  a drawn refusal carries far more authority than a sketched one, and nothing visible in the answer backs
  it. Note that the same login appears in both halves: the account that keeps a read here is
  `novasmart-customer-sa`, and what changed is the **scope** — from read, change and delete down to read
  only. Name the AFTER principals from the live `access[]` you just re-read rather than from the shape
  described here. If the identity split was blocked in Part 2, the principal you removed is the shared
  login, not a new SA; show what you did, not what the shape suggests.
...
- **Don't mislabel:**
  ...
  - Don't claim the promo agent is "blocked" yet. Configuration says it should be; **Step 3's audit log is
    what proves it.**
- **How to close STEP 2 — this is the step's ONE close, and it covers all three parts.** A model of the
  *shape*, re-derived from what you read this turn, never recited. The honest gap is the distance between
  a policy and a demonstration: the access list now says what you want it to say, and a list is a
  **record of a decision, not a demonstration that the decision took effect** — and those two have parted
  company before. `What this does not fix` carries that, alongside the Part 2 point that attribution is
  not approval — being able to trace an access does not make it appropriate. `Worth sitting with` asks
  what NovaSmart would accept as evidence that a removal actually bit, what its standing rule ought to be
  for access that is now visible, or what else in the estate is trusted on the strength of a list nobody
  has tested.
...
> **🔒 The Step 3 gate — two hard requirements, both satisfied *before* any claim leaves your mouth:**
>
...
- **What good looks like:** the fixed verification table (§9), with the acting principal quoted
  **verbatim** — `principalSubject` for an Agent-Identity workload, `principalEmail` for a service
  account — reads that now name **one** workload each, and a real 403 entry for the promo agent. Then a
  bottom line that states **only** what the rows show.
- **Don't mislabel (this is where a confident-sounding answer goes wrong):**
  - ⛔ **Name the gate the denial came from — quote `protoPayload.authorizationInfo[].permission` and
    ...
    the two claims Part 3 of Step 2 just spent a paragraph separating, and it credits the wrong control with the
...
- **No picture here — FORBIDDEN, and this is the step where it matters most.** The answer *is* a
  ...
  picture of a proof is where a tick nobody earned gets laundered into something that looks read — and
  a **rendered** picture launders it far better than a sketch ever did, because it arrives looking like
  a finished artefact with nothing visible behind it. If the leader expects a picture, say in one line
  why there isn't one: this is a result, and a result is shown as rows you can trace to a log entry.
...
> **🔒 This step AUDITS what the earlier steps did — it does not do any of it, and it does not repair
> any of it.** ⛔ **ZERO MUTATIONS:** no binding added or removed, no service account created, no service
> re-pointed, nothing re-run to turn a row green. If a control is missing, **the finding is the output**:
> name the step that would put it right and hand it back to the leader. Quietly fixing it here would hide
> from them that it was ever missing, and that is the one thing this step exists to surface.

- **Where to look — every read is one §9 already names, run again NOW.** The registry listing for the
  shadow agent · `describe` on **both** workloads' runtime identity · the project policy filtered on
  `novasmart-customer-sa` · `bq show` on `customer_data` · and both audit-log queries from §5, the
  `tableDataRead` one and the `status.code=7` one. Nothing new is being established here; §9's table is
  being filled from commands you ran **this turn** instead of from what an earlier step reported.
- **A value an earlier step quoted is a value from *then*.** If you did not read it again in this turn,
  that row is **not verified**. Currency is the only thing this step adds, so borrowing Step 2's or
  Step 3's output and presenting it as an audit result empties the step out.
- **What good looks like:** §9's table with **every** row present, each Result carrying either a value you
  quoted this turn or the words **not verified**, the row count stated out loud, and the count of
  `not verified` rows said plainly (§9). ⚠️ **A table with gaps is this step working, not failing.**
- **Don't mislabel:**
  - ⛔ **Never emit the §9 scorecard block with a placeholder still in it.** It is a shape to fill, and an
    unfilled one reads as an all-clear nobody earned — the exact failure this module has produced before.
  - **A check you could not run is `[ NOT CONFIGURED ]`, `[ FAILED ]` or `not verified` — never a pass.**
    An empty query result is not a denial and a missing log entry is not evidence of one (§4 · Step 3).
  - **Configuration is not demonstration.** An access list that reads correctly says what the policy is
    today; only a quoted log entry says what the platform actually did when someone knocked.
  - ⛔ **Attribution is not exhaustive.** The reads you *observed* named one workload each; that is not the
    same claim as every read of customer data being attributable. Say **which** reads, in which window,
    and leave the MCP masking effect (§1, §4 · Step 3) named rather than rounded away.
  - ⭐ **Always run the updater, and let `--status` carry what you measured.** `python3 $HOME/Desktop/Session1/.agents/skills/novasmart-governance-lab/scripts/update_scorecard.py
    --mission M1 --status <PASS or FAIL>`. **`PASS` only when every row carried live evidence this turn;
    `FAIL` the moment one row reads `not verified`, `[ FAILED ]` or `[ NOT CONFIGURED ]`.** ⛔ **Not
    running it is not the safe option** — a mission that was verified and came back short must show on the
    board as short, not as absent. Absent reads as never attempted. What a false all-clear costs is the
    wrong `--status`, not the fact of writing a row.
- **No picture here — FORBIDDEN.** This is a verification result, and §9's three-column table is the only
  permitted form (§4 · Step 3).
- **How to close:** one line stating only what the rows show, with the `not verified` count in it — then,
  **only if every row is carrying live evidence**, the filled scorecard block and the achievement line
  from §9. If any row is unverified, say so **before** the block and do not let the block's tidy shape
  imply otherwise.
...
- ⛔ **This step is not the optional change-disclosure prompt.** *"Show me everything you changed today.
  ...
  answering it is **not** Step 5, and titling that answer as Step 5 misreports what the module covered.
  **If the leader never drives Step 5, record it NOT RUN** rather than folding another turn into it — the
...
- **How to close** — a model of the *shape*, re-derived from what this session actually produced, never
  ...
  ⛔ **Never sign off with a completion notice, and never name a scaffolding file of your own.** A real
  run ended a module with "all steps are now complete and fully logged in `LAB_RUN_LOG_M1.md`" — that
  declares victory, leaks scaffolding, and gives the leader nothing. **What is banned is the completion
  notice standing in for a finding, not the act of naming a file:** the step's own evidence file is the
  leader's, it says what is in it, and pointing at it is required — so point at it, and never let the
  pointer be the close. ⛔ No "the estate is secure", no "fully governed", no five-of-five tally. Nobody
  is scoring the module green.

---

## 5. The commands that actually work here

- **Move an agent to a native Agent Identity (Step 2a) — verified live 2026-07-31.** One PATCH, a
...
- **Read the new identity (Step 2b) — never construct it (§8):**
...
- **Re-key the access onto the new identity (Step 2b) — both parts, verified live:**
...
- **Resolve once, cache for the session** (`PROJECT=$(gcloud config get-value project)`, project number via
  `gcloud projects describe`, workloads via `gcloud run services list`). The shadow's URL **and** current
  service account come from `gcloud run services describe promo-agent-shadow --region=<REGION> --format=yaml`
...
- **Register it (Step 1)** — **`--location` is required, and it is NOT `global`.** Writable services are
  ...
  **Register `promo-agent-shadow` into `us-central1`:** it is the region of the Cloud Run service the entry
  ...
  governance plane does not look. (`us-central1` is this estate's region — resolve it from
  `gcloud run services list` rather than assuming, if the environment differs.)
...
- **Give the shadow its own identity (Step 2):**
...
- **Read the shared login's power (Step 2, Part 1 — read-only)** · `bq show --format=prettyjson "${PROJECT}:customer_data"`
  ...
  **That first `bq show` on the dataset is needed for Step 2's opening readout as well as for the
  right-size later in the same step** — in the right-size it is the read half of the read-modify-write
  below. So a permission problem with it does **not** wait for the right-size: it bites **at the very
  start of the step**, in the readout's own evidence-gathering, and without it you cannot show the leader
...
- **Remove the over-broad grant (Step 2)** — removing *someone else's* excess is the fix; adding a role to
...
- **Dataset-scoped access (Step 2, Parts 2 and 3)** — a read-modify-write on the whole `access[]`, so re-read after:
  ...
  **This one grant on `novasmart-customer-sa` is expressly allowed** — it is exception (ii) of the §6·5
  allowlist, the mandated Step-2 right-size. §6·5's ban is on *other* principals, not on this fix.
...
- **⚠️ The Step-2 right-size is ONE transaction, not two independent edits.** It has two parts, and they
...
- **The documented atomic alternative: `GRANT … ON SCHEMA`.** BigQuery's DCL grants a dataset role in a
  ...
  `REVOKE … ON SCHEMA … FROM …` is the matching removal for Part 3 of Step 2. Prefer this path when it works. It runs
...
- **Trigger the legitimate read for real (Step 2b's end-to-end check and Step 3's part 1) — this is the
...
- **Prove it from the audit log (Step 3) — two queries, one clause apart:**
  ...
  verified**, never ✅.
- ⚠️ **A BLANK `principalEmail` column after the Step-2 flip is the EXPECTED result, and it is evidence
...
- **Change record — one per change, fixed fields, appended to that step's evidence file.** The four
  fields do not change — **Change · Resource · When (UTC) · Exact command to undo** — but in a plain-text
  file they are written as a **block, one field per line, with the undo command unindented on a line of
  its own**, not as a table: a pipe table is unreadable at 80 columns and an undo command trapped in a
  table cell is not cleanly copy-pasteable, which is the whole reason the record is in a file. Several
  changes in one step means several blocks in that step's file. A step that changed nothing still prints
  the section, with `(nothing changed in this step)` in it — an absent section is indistinguishable from
  a dropped one. **The answer, separately, carries one plain line per change** (§4 Step 2).
...
5. **🚫 The IAM allowlist — judged by WHO receives the role, never by WHY you want to grant it.** In the
   ...
   - **(ii)** the **Step-2 right-size on the shared account `novasmart-customer-sa`** — the one account
   ...
   - **(iii)** the **agent identity THIS step just minted** — the `principal://` you read from
     `spec.effectiveIdentity` after the Step-2a flip, and **only** the two roles the re-key needs:
     project-level **`roles/bigquery.jobUser`** and **dataset-scoped `READER`** on `customer_data`
     (§4 Step 2b, §5). **This grant is not optional and not a loophole — it is the second half of the
...
6. **🚫 NEVER re-point a running workload's identity, with two named exceptions.** Changing which service
   ...
   (a) attaching the **new service account you just created for that same workload** in Step 2, and
   (b) flipping the **Customer Personalization Agent** from `novasmart-customer-sa` to **Agent Identity**,
   which is what Step 2 mandates — it is a `PATCH` of `identityType`, takes about 45 seconds and needs
...
7. **✅ If you cannot create a service account — THIS is the legal path, and taking it is the right
   ...
   blocker that can stop Step 2's identity split. When you hit it, after the cheap retries in steps 1–3:
   ...
   3. **Mark that check "not verified"** in §9 — never ✅ — and **move on to the next step**, doing the
      parts you still *can* do. The Step-2 right-size of `novasmart-customer-sa` is independent of the
      split and still applies; so does the rest of Step 2, and so do Steps 3, 4 and 5.
   ...
   > governance report this module is teaching the leader to expect. Nobody is scoring you on getting
   > every step green.
...
**These rules govern this step's evidence file — `m1/m1_step<N>.txt` — and every value you lift out of it
into the answer. The evidence is no longer in the answer; the standard it is held to has not moved, and
two of these rules get *stronger* now that the raw output is a file you can literally re-open.**
...
- **Every change carries a change record** — what changed · on which resource · when (UTC) · the exact
  command that undoes it, appended to that step's evidence file, with one plain line in the answer saying
  it happened. No record, no "done".
- 🔎 **The outputs↔commands rule — mechanical, and checkable by anyone who opens the file.** **Every
  output recorded in the file MUST belong to a command that appears, verbatim, in that same entry's
  commands section**, and every command in that section is one you actually ran, exactly as you ran it.
  If an output has no command behind it, **it is invalid** — delete it, or actually run the command and
  paste what it returned. There is no exception for a command you *would have* run, a reconstructed
  invocation, a tidied-up flag order, a shortened path, or output remembered from an earlier turn.
  Measured on 2026-08-04: **seven of nine turns** carried evidence that was composed rather than
  captured, and **two were provably false** — one showed invented `head -n 12` output that disagreed with
  the real file on two fields, and one showed output for a command that appeared nowhere in its own
  turn's command list. ⛔ **Composed output is fabrication even when it happens to be right**, because
  nothing lets the reader tell the two apart — and the one time it was wrong, nothing caught it.
  ⚠️ **This rule gets stronger, not weaker, in a file.** The command list and the output it produced now
  sit in one place, on disk, in a form anyone can diff — which is the check the old inline arrangement
  only ever asked agy to perform on itself.
- **Every value in the visible answer must be in the file's outputs section** — the part you did not
  author. That is the parity rule from §0, and it is what replaces the leader's old ability to look
  directly beneath a claim and see the output that backs it. **Carve-outs, so it stays usable:**
  plain-English glosses, the step title, the leader's own words quoted back, **a wait and its duration**
  (that is a record of *your own* conduct, not a system output — state it in the answer and label it as
  yours), and the words *unknown*, *not verified* or *no evidence recorded*, which are statements about
  the absence of output.
- **Write the file before you draft the answer, and lift every value out of a re-read of it** — not out
  of memory. This is the same move this section already makes for mutations, applied to the answer
  itself, and it costs nothing: the entry's own written-at stamp must come before any timestamp the
  answer claims for itself.
- **The file is append-only.** A step answered twice is a **new entry**, never an edit of the old one,
  and a correction is a new entry that says what it corrects. A record you can rewrite after the fact is
  a draft, and the file's entire value is that it is a record.
- **Log every non-OK tool result, including the ones you worked around.** A non-zero exit, a
  `PERMISSION_DENIED`, a 4xx/5xx, a timeout or an empty result **is recorded in that step's file even
  if you retried past it, routed around it, or it turned out not to matter** — which is why every output
  in that file carries its exit status, successes included: a status line on everything is what stops a
  silently-recovered failure from simply not being there. Measured on 2026-08-04: a
  `PERMISSION_DENIED` on `bigquery.jobs.create` by `antigravity-sa` at **02:56:33 UTC** — inside the
  identity split's
...
- **`--location` is mandatory** on `agent-registry` commands, and **two locations are in play**: the
  **regional** one — `us-central1` here, where the deployed agents live **and where you register the shadow
...
- **`bigquery.jobUser` has to be project-level** (it grants job creation, not data). Say that when you
  ...
  violation:** granting `roles/bigquery.jobUser` to **`novasmart-customer-sa`** as part of the Step-2
...
- **Propagation lag is real in both directions, and it is longer than it feels.** An IAM change typically
  ...
  **The measured floor here is about 90 seconds** (refused at 20 s, allowed at 90 s), which is why the
  wait is a **step instruction** in §4 Step 2 and not merely background reading. **The specimen this
...
## 9. Verification checklist for M1
...
Fixed shape — **`Check | The command/observation that proves it | Result`**. Three columns, and the middle
one must name a command you **actually ran this session**.

⛔ **The row set below is FIXED. Never shorten the table, and never drop a row you could not perform.** If
you did not run the middle column, **keep the row** and write **`not verified`** in Result, together with
the command you tried and the error it returned. An unevidenced ✅ is a fabricated finding; a **silently
missing row is worse**, because nobody reading the table can see what you did not check. In a real run two
rows simply vanished — *the legitimate path still works* and *a real read now names exactly one workload* —
with no marker and no mention, and the table read as a clean pass.

⚠️ **A table with several `not verified` rows is the module reporting accurately, not the module
performing badly.** Do not "tidy" it by deleting the gaps, and say so in your close if the count looks
unflattering. The `not verified` rows are the honest half of the result.

✅ **The mechanical check, before you send it: count the rows in the table below, count the rows in the
table you are about to emit, and state both numbers in one line.** If they differ, your table is
**truncated** — go back and restore every missing row with **`not verified`** in Result. This is not
belt-and-braces: in a real run this table shipped with **three** rows, every one of them marked verified
and **not a single `not verified`** anywhere, with no marker and no mention that the rest had been
dropped. It read as a clean pass over checks that were never performed. **`not verified` is the DEFAULT
for every row you did not re-read live in this session** — write it in first, and replace it only once
you are holding a quoted value from a command you actually ran.

The **Result** column carries what you observed: a quoted value, or the words **not verified**. Never a ✅
you did not earn.

| Check | What proves it (run it, then quote it) | Result |
| :-- | :-- | :-- |
| Shadow agent is **registered with a named owner** | `gcloud agent-registry services list --location=us-central1` — **the same location you registered into, never `global`** (§5), and the **re-read**, not the create response | the entry you re-read, or **not verified** |
| The promo agent runs under **its own** service account | `gcloud run services describe promo-agent-shadow --region=<R> --format=yaml` → its SA **and** traffic on that revision | the account and the traffic split you read back, or **not verified** |
| The two workloads **no longer share** one identity | the same `describe` for **both** workloads, side by side, showing different principals | the two principals, quoted, or **not verified** |
| The project-level `bigquery.admin` on the shared SA is **gone** | `gcloud projects get-iam-policy … --filter="bindings.members:novasmart-customer-sa"` → no `roles/bigquery.admin` | the filtered role list you read back, or **not verified** |
| The agent that genuinely needs customer data has **dataset-scoped read-only** | `bq show --format=prettyjson customer_data` → its entry is `READER`, and there is no `OWNER`/`WRITER` for it | its `access[]` entry, quoted, or **not verified** |
| The promo agent has **no path** to customer data | the same `bq show` (its principal absent from `access[]`) **plus** the project policy filtered on that principal | what the two reads showed, or **not verified** |
| A real read now names **exactly one** workload | the `tableDataRead` query (§5) — the acting principal quoted verbatim: **`principalSubject`** (the `principal://…` value) for a workload on an Agent Identity, `principalEmail` for a plain service account. A blank `principalEmail` beside a populated `principalSubject` is the **expected** post-flip shape, not a miss (§5) | the acting principal you read, or **not verified** |
| The promo agent's read is **actually denied** | the `status.code=7` denial query (§5) — quote the entry. Absent after a retry → **not verified**, never ✅ | the entry you read, with its gate and resource (§4 Step 3), or **not verified** |
| Change/delete on customer data is blocked | **Only** if you actually attempted it (a non-destructive probe, e.g. `DELETE … WHERE FALSE`) **and** quoted the denial. Otherwise state it as *configuration* evidence — "IAM shows read-only" — and label it as such, never as a live block | the denial you quoted, or "configuration only", or **not verified** |
| The **legitimate** path still works | you triggered it with the A2A `message:stream` call in §5 — **not** the Store Portal, which reads as its own account — and the successful read appears in the log under the agent's identity | the log entry you found for it, or **not verified** with the call you tried and its error |
| A **change record** exists for every change | your own records: what · resource · when (UTC) · exact undo command | the records you wrote, or **not verified** |
| Nothing outside M1's scope was touched | re-read the project policy: project-wide `aiplatform.user` and `test-agent-caller`'s bindings are **unchanged** (§3) | unchanged, or changed — say which |
| **`test-agent-caller` is exactly as you found it** | project policy filtered on it (no new roles) **and** `gcloud run services list --format="table(metadata.name, spec.template.spec.serviceAccountName)"` — it is **not** the runtime identity of any service (§3, §6·6) | unchanged, or changed — say which |
| The promo agent's SA is one **you created this session** | `gcloud iam service-accounts describe <SA>` — if you could not create one, this row is **not verified** with the §6·7 blocker report beside it, never a substitute account | the describe output, or **not verified** with the §6·7 report |
...
emit a false all-clear, and never let a clean-looking summary imply a check you skipped. **Count the
`not verified` rows out loud rather than hiding them** — that count is part of the result.

### The Mission 1 scorecard — a form to fill, never a result to emit

**Model shape only — every bracket below is a blank.** Fill each one from a row of the table above, using
the same word you wrote in that row's **Result** cell: `PASS` **only** where you are holding a value you
quoted this turn, otherwise `NOT CONFIGURED`, `FAILED` or `not verified`. The tally is counted from the
filled rows, never stated in advance.

⛔ **Never emit this block with a placeholder still in it, and never emit it at all without the table
above it** — a scorecard that arrives without the rows it summarises is a fabricated all-clear, and this
module has produced one before.

```
========================================================================================
🛡️ MISSION 1 COMPLETION SCORECARD · IDENTITY & DATA LEAST PRIVILEGE
========================================================================================
Identity Isolation: [ RESULT ] <what the two describes showed about the shared login>
Dataset Scope:      [ RESULT ] <the customer_data access[] entry you quoted>
Promo Agent Access: [ RESULT ] <what the policy showed, and what the denial query returned>
Audit Attribution:  [ RESULT ] <the reads you observed, and the principal each one named>
----------------------------------------------------------------------------------------
Overall Status:     [ STATUS ] <n> of <total> checks evidenced; <n> not verified
========================================================================================
```

⚠️ **`Audit Attribution` is a statement about the reads you observed in a stated window — never "100% of
customer data reads".** This estate has principals that read that data outside the two agents in the
story, and the tool layer can read it with its own credentials (§1, §4 · Step 3), so a total is a claim
you cannot evidence from any query this module runs.

🏆 **Achievement Unlocked — emit only if every row above carries live evidence:** *Eliminator of Shared Credentials — You split the shared login and gave each workload its own identity on the customer dataset.*
