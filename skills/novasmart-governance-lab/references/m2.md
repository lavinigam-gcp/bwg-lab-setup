...
>
> **M2 has exactly ONE inbound mutation** — a rewrite of the **back-office agent's own IAM policy**, in
> the second half of **Step 2**. Step 1 before it is read-only, so is the first half of Step 2 itself, and
> so is Step 3 after it. That single write is small, precise
...
> satisfying one (§0 Rule C, §1, §4 · Step 3).
...
> **Step 1** See who can call the back office · **Step 2** Check the cost, then lock it to the front desk
> *(reads and shows the impact FIRST, then the one inbound write)* · **Step 3** Prove the rogue caller is
> out · **Step 4** Lock down what the back office can reach and do · **Step 5** Verify M2 connection
> controls & generate Mission Scorecard *(a read-only audit)* · **Step 6** What's next.
...
- **Rule C — a resource allow-policy cannot take away a project-level grant.** This is the single most
  ...
  **additive and evaluated as a union**: the permissions a principal has on the back-office agent are
  ...
  ⚠️ **Rule C governs the resource hierarchy, and one binding in Step 4a sits outside it.**
  ...
  **non-additive**: a per-resource binding **replaces** the registry-wide one for that resource rather
  than merging with it. That is why Step 4a says **registry-wide** and means it — a per-resource grant
...
They are written to **this step's own evidence file** — `m2/m2_step1.txt`, `m2/m2_step2.txt` and so on,
...
**What stays in the answer is the finding, never the raw material** — the status you got, the count you
read, the members the list named, the sentence that says what it means. Beneath it goes one short line
saying where the file is and how many commands it holds.

⚠️ **Figure parity — the rule that replaces "the output is sitting right there."** Every literal value in
the visible answer — a status code, a count, a role name, a principal, an `etag`, a timestamp, an engine
ID — **must also appear, character for character, in the outputs section of that step's entry.** If it is
not in the file, it did not come from a command, and it does not go in the answer. The carve-outs are the
plain-English glosses, the leader's own words quoted back, waits you state about your own conduct, and the
literal phrases **not verified** / **unknown** / **no evidence recorded**, which are statements about the
*absence* of output.
...
wrote. It is never a figure carried in from this file, and it never rounds a gap away.

### ⛔ The exceptions: Steps 1, 3 and 5 change nothing, and nor does the first half of Step 2

| | Step 1 | Step 2 | Step 3 | Step 4 | Step 5 |
| :-- | :-- | :-- | :-- | :-- | :-- |
| **Do you change configuration?** | **No.** You read policies and you make one call as the rogue login. | **First half, no** — you model the impact and put it in front of the leader. **Then yes** — the one inbound mutation, and never before that readout exists. | **No.** You re-make the same calls and read verdicts. | **Yes** — the gateway attach and the database narrowing. | **No — ZERO MUTATIONS.** It is a read-only audit of what the earlier steps left behind, and it repairs nothing it finds missing. |
| **Do you ask permission?** | **No.** | **No.** | **No.** | **No.** | **No.** |
...
the worst failure available in this module, because Step 3's whole proof is a **replay** of it.

**Step 2 opens on the judgment beat, and that beat does not disappear because the same prompt also orders
the lock.** The leader is meant to work out for themselves that "lock it down" has a cost worth checking
*before* the lock goes on, and to discover that one of the callers on the list is their own storefront.
**Show the impact in full first; apply second.** If you apply the lockdown while explaining what it would
have cost, you have deleted that moment — and the merged prompt makes that the easy mistake, because
nothing external now separates the two halves. The impact half ends with the judgment question and you
then carry straight on: it does not end with a request for approval, and there is no pending plan in
between.
...
- **`roles/aiplatform.expressUser` is not the narrow substitute it looks like.** It contains **query,
  ...
  `aiplatform.reasoningEngines.query`** (§4 · Step 2 tells you when that is and is not worth doing).
...
would get, never about what you would change. Nothing is conditional on whether they use one; you do not
wait, you do not follow up, and you never mention it again.

| Step | The prompt (copy it exactly) | What they would see (one line, under the prompt) | Steering if they type it |
| :-- | :-- | :-- | :-- |
| **Step 1 · See who can call the back office** | `What does the back-office agent actually hold that makes this worth protecting?` | "This reads the agent's own description and the data it is wired to, so you can see what is behind the door before we talk about the lock." | Read the deployed agent's description/instruction and the datasets it is configured to reach, and report what those reads returned. ⛔ Change nothing (§3). ⛔ **Do not name the lockdown, the fix, or which caller would be allowed** — that is the next step's own readout. ⛔ Do not quote the margin data itself; naming the kind of data is the answer |
| **Step 2 · Check the cost, then lock it to the front desk** | `If someone added a name to that list tomorrow, would anyone find out?` | "This checks whether anything in the estate is set up to notice a change to that access list on its own, and the honest answer here may well be nothing." | Read what actually exists: whether admin-activity logging captures a `setIamPolicy` on this resource, and whether any alert policy or notification channel is configured. Report the count you measured, **including zero**. ⛔ **Do not build the alert** — naming what should exist is the remedy-shopping this block exists to avoid; say what is absent and what its absence costs, and stop. ⛔ Do not point the leader at the monitoring dashboard (§1) |
| **Step 4 · Lock down what the back office can reach and do** | `Which other agents hold a broad database role like the one you just narrowed?` | "This lists every agent identity in the project against the database roles it holds, so you can see whether the one we narrowed was unusual or typical." | Re-read the project policy this turn and report, agent by agent, the roles their `principal://` identities actually hold. Say which you could **not** resolve. ⛔ **Remove nothing and grant nothing** (§3, §6·5) — the list is the whole deliverable, and narrowing a second agent is an unasked-for change nobody has sized. ⛔ Quote no count you did not read this turn |
| **Step 4 · Lock down what the back office can reach and do** | `Does that gateway keep a record of what it let through, or only what it stopped?` | "This looks at what the gateway itself writes down, which is the difference between being able to answer 'what happened last Tuesday' and not." | Read the gateway's own log entries (§5) and report what the verdict field actually carries for allowed and for denied destinations, with the query and the window you used. ⛔ Do not change the policy, the extension or the gateway to make the record tidier (§3). ⛔ If you observed no entries, say so and name the query — an empty log is a finding, not a verdict |

⛔ **Steps 3, 5 and 6 supply no sideways prompts, deliberately.** Step 3 and Step 5 are verification turns
where anything typeable would compete with the evidence, and Step 6 is a recap. Emit no block on those
steps and do not apologise for it.
...
- ❌ **IAM deny policies, org policies, VPC-SC, or any other subtractive control.** These are the
  ...
  answer and **building one here is not** (§4 · Step 3).
...
> Step 3's before/after, so **save the exact URL, the exact request body, and the raw response with its
...
- **What good looks like — a fixed shape, filled only from live output:**
  ...
  **The `Which command showed me` cell is compulsory and it is not decoration** — it is the same
  ...
  command anywhere behind it**, while the picture beside it correctly showed the same route as unverified.
  ...
  **One row is required whatever else you found: the project-wide holders.** Its `Who` cell names the
  ...
  caller** — it is the largest group on the list and the reason the honest claim in Step 3 is narrower
  than the tidy one.
...
- **The picture — REQUIRED here, and it is a CALL picture (§2).** Show the one distinction the whole
  module turns on: who is on the back office's **own** caller list, and who gets in **without being on
  it**. **Exactly one route is drawn as something that happened** — the rogue call you actually placed,
  which is the only call this step makes. **Every other route is drawn as something merely permitted**,
  because you read a permission in a policy and you did not watch anybody use it. Show those as observed
  calls and you have claimed calls you never saw.

  **What the picture must contain, and nothing else:**
  - the back-office agent as the destination, named as the live listing named it;
  - a first group headed as the agent's **own** caller list, holding the members you read out of
    `:getIamPolicy` — the rogue among them, marked as the one route you exercised, with the status it
    returned;
  - a second group headed as callers who reach it **without** being on that list — the front desk, the
    store portal's direct route if you found one, and the project-wide role holders **as a count you
    read live this turn** — each marked as permitted-but-unobserved;
  - a legend saying in plain words what the two markings mean, and one line naming what would settle an
    unobserved route: place the call and read the status;
  - a caption naming what you read, from where, and when.

  **The count of project-wide holders is whatever you counted in the live project policy** — never a
  number carried over from this file, and never the word "ten" from §1. The two groups are the point: the
  second one is the reason the honest claim is narrower than the tidy one, and putting it in the picture
  is what stops you writing the tidy one in the write half of Step 2 or in Step 3. Anything you did not
  actually read is marked **unknown**, with one line naming the command that would settle it — a group
  you omit reads as "there is nobody there", which is a claim you cannot support. If the storefront's
  direct route turned out not to exist, leave it out and say why; never show a route you did not find.

  **⛔ Do not upgrade an unobserved route to an observed one here, however obvious it looks.** The front
  ...
  observation. Step 3 is the first step that exercises the front desk for real, with its own before/after.
  The store-portal route is weaker still: it comes from the application's own configuration, which is
  known to carry a **stale hard-coded engine id** (§8), so mark it as unobserved and say in the caption
  where you read it. If you cannot say which command produced a route, that route does not go in — **and
  the same rule governs the table above it**: a row whose `Which command showed me` cell is empty is a
  claim, not a finding, and it does not go in either. The picture and the prose are held to one standard.
- **Don't mislabel:**
  - **A call you did not make is not a finding.** If you describe the rogue call without making it, you
    have fabricated the step — and destroyed Step 3, which can only replay something that happened.
  ...
  - **Don't call the gateway a control yet.** At Step 1 it is attached to nothing and enforces nothing.
    It becomes a control in Step 4, and only once you have read a verdict back (§1, §7).
...
- **How to close — a shape to re-derive, never a line to recite.** What this step actually taught the
  ...
  *that*, not the next thing to do. The **ideas**, in whatever order your own reads support:
  ...
  escalation actually returns a decision is a behaviour, it is Step 3's to observe, and asserting it here
...
#### Half two · Lock it to the front desk — the one inbound mutation

- **This module answers two questions, and they need two different controls.** *"Who may call the back
  office?"* is answered by the access list on the back-office agent — that is this half, and it is
  unchanged. *"Which destinations may the back office itself reach?"* is answered by the **Agent
  Gateway**, in **Step 4**. ⛔ **Neither substitutes for the other.** Do them in order, report them
...
- **Test the legitimate path through the FRONT DESK, not through the bypass.** This is one of the two
  easiest ways to get Step 3 wrong: the storefront has a route that calls the **back office directly**.
...
- **⚠️ The price you quote must come from the seeded competitor data — this is the other easy way to get
  Step 3 wrong, and it is the more likely one.** The front desk does **not** reason about your numbers.
...
> **Two controls, not one, and they answer different questions.** Step 2 settled who may call the back
> office. This step settles **where the back office may go** and **what it may do when it gets there** —
> the gateway for the first, the agent's own database permissions for the second. ⛔ **Never present
> either as covering the other's question** (§7).

#### Step 4a · Put the back office behind the gateway — which doors it may go through
...
#### Step 4b · Narrow what it may DO there — read yes, write no

- ⚠️ **The gateway cannot do this part, and Step 4a's verdict does not cover it.** A gateway allow means
...
- ⛔ **Both parts of this change, or the agent is stranded.** The job-running role grants the right to *run* a query
...
- **Don't mislabel — and the first one is the trap this whole module is built around:**
  ...
  - **Never emit the §9 scorecard block with a placeholder still in it**, and run
    `python3 $HOME/Desktop/Session1/.agents/skills/novasmart-governance-lab/scripts/update_scorecard.py --mission M2 --status <PASS or FAIL>` .
    ⭐ **Always run it; `--status` carries what you measured.** `PASS` only when every row carried live
    evidence this turn; `FAIL` the moment one reads `not verified`, `[ FAILED ]` or `[ NOT CONFIGURED ]`.
    ⛔ **Skipping it is not the safe option** — a mission verified and found short must show on the board
    as short. Absent reads as never attempted; what a false all-clear costs is the wrong `--status`, not
    the fact of writing a row.
...
- **The gateway sequence (Step 4a) — every line below was run live 2026-08-01:**
...
- **The narrowing (Step 4b) — measured 2026-08-01, and verified against the table itself:**
...
- **Resolve the two engine IDs and the PMA principal (Step 1 & Step 2).** Prefer the **live** listing;
...
- **Who can call an agent, project-wide (Step 1) — two reads, and you need both:**
  ...
  #    promises the leader the list (§4 Step 2, Step 3, §9). ONE policy read, filtered
  ...
  **Read the list out loud; do not summarise it.** **What to look for as you read it:** any member that is
  ...
  satisfy §4 Step 3** — the permitted sentence there ends *"here they are"*, and that clause is a promise
...
- **Mint the rogue's token WITHOUT disturbing your own credentials (Step 1 & Step 3):**
...
- **The rogue call — identical in Step 1 and Step 3 (the body shape the storefront itself uses; confirm
  live):**
  ...
  **Save the URL, the body and the response both times.** Step 3 must differ from Step 1 in exactly two
  things: the clock, and a freshly minted token.

- **⚠️ The one inbound mutation (Step 2) — read, edit, write back with the `etag`:**
...
- **Rollback (put it in the change record in this step's evidence file, verbatim — §0):** re-apply the bindings from
...
- **The legitimate escalation (Step 3) — through the FRONT DESK:** send a genuine **>10%** discount
  ...
  **⚠️ Quote a competitor price the data actually contains, or the request is denied before the discount
  ...
  do with your lockdown (§4 · Step 3). The verified pair on the standard test item:
...
- **Audit-log corroboration (Step 3) — discover the shape first, then narrow.** Don't paste a filter you
  haven't seen match:
  ...
  — say so, and lean on the HTTP refusal you caused yourself (§4 · Step 3 evidence ranking).
- **Change record — one per change, appended to this step's evidence file (§0), never a pipe table.** A
  ...
  Several changes in one step means several of these blocks in the same entry. If nothing changed in a
  step, the section is still written, reading `(nothing changed in this step)` — an absent section is
...
7. **✅ If you cannot write the policy — THIS is the legal path, and taking it is the right answer.**
   ...
   3. **Mark the affected checks "not verified"** in §9 — never ✅ — and **move on**, doing the parts you
      still can: Step 1's exposure evidence stands, Step 2's impact analysis stands, and Step 3 becomes an
   ...
   > M2 answer. Nobody is scoring you on getting all six steps green.
...
- ⚠️ **Every element of a picture must correspond to something a command returned in this session.** A
  generated picture invents willingly and convincingly: a real run produced one carrying a `Workspace
  Agent` and a `Gemini Core`, **neither of which exists in this estate**, and both read as fact because
  they were rendered rather than written. So before you send a picture, walk it element by element and
  name the command behind each one — the agent names from the live listing, the members from
  `:getIamPolicy`, the count from the project policy read. An element you cannot trace is marked
  **unknown** with the command that would settle it, or it comes out. **A plausible name you did not read
  is a fabricated finding, whatever it is drawn in.**
...
- **Before you send:** re-read your draft against the raw output in this step's evidence file and delete
  ...
  up the answer, and it is the same check. Then delete every sentence claiming exclusivity (§4 · Step 3).
...
- **A 403 from the *wrong* URI looks exactly like a 403 from your policy.** Confirm the region host, the
  project, the engine ID and the API version are byte-identical between the Step 1 and Step 3 calls before
...
columns every verification in this module uses**, including Step 3's own readout (§4 · Step 3): there is
...
**filled table is written into the verify step's evidence file** (§0) — every row, in this order,
including the ones marked `not verified`. **The answer carries one coverage line instead**: how many
checks you evidenced and how many you marked not verified, counted off the rows you actually wrote.
⛔ Moving it changes nothing about what has to be in it. **Every row still ships**, every Result cell is
still filled from live output, and shortening the table because nobody is looking at it in the answer is
the same fabrication it always was.
...
unverified, say so plainly — never emit a false all-clear, and never let a clean-looking summary imply a
check you skipped. **"The rogue caller is refused and the back office now has an access list of one" and
"only Price Match can call the back office" are different sentences; only the first one is true.**

### The Mission 2 scorecard — a form to fill, never a result to emit
...
above it** — a scorecard that arrives without the rows it summarises is a fabricated all-clear.

```
========================================================================================
🛡️ MISSION 2 GOVERNANCE SCORECARD · A2A ACCESS CONTROL & EGRESS FILTERING
========================================================================================
A2A Inbound Policy:      [ RESULT ] <the members the back office's own list named>
Rogue Access Denial:     [ RESULT ] <the status and error body the replay returned>
Legitimate Escalation:   [ RESULT ] <the margin decision the front desk gave back>
Egress Gateway Policy:   [ RESULT ] <the gateway verdict you quoted, and for which host>
----------------------------------------------------------------------------------------
Overall Status:          [ STATUS ] <n> of <total> checks evidenced; <n> not verified
Still uncontrolled:      <the project-wide invoke holders you counted this turn>
========================================================================================
```

⚠️ **`A2A Inbound Policy` records who the agent's own list names — never that invoke is "restricted to"
the Price Match Agent.** Allow policies are additive and the project-wide holders are untouched, so an
exclusivity claim in that row is the same false sentence this module bans everywhere else (§0 Rule C,
§1, §4 · Step 3). The `Still uncontrolled` line is what keeps the block honest; it is not optional, and
its number is counted live in the turn you emit it.

🏆 **Achievement Unlocked — emit only if every row above carries live evidence:** *Architect of Agent Boundaries — You locked the back-office agent's inbound calls to the front desk alone.*
